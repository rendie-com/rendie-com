    ...
    d02: function (request, next) {
        switch (request.action) {
            ...
            case "tabs_remove_create_indexOf"://删除后创建再查找(返回网页内容)
                common_executeScript.tabs_remove_create_indexOf.a01(request.index, request.windowId, request.url, request.html, request.isHighlightTab, next);
                break;
            case "tabs_executeScript_indexOf"://执行js代码后再找内容(返回网页内容)
                common_executeScript.tabs_executeScript_indexOf.a01(request.index, request.windowId, request.fileArr, request.code, request.html, request.isHighlightTab, next);
                break;
            ...
            case "postHtml": common_fetch.postHtml(request.url, request.data, next); break;//普通提交（返回的内容不一定是json格式
            ...
            //     common_fetch.setHeaders_postHtml.a01(request.url, request.headers, request.data, next);
            //     break;

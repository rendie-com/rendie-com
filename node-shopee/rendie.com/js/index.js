    ...
    d02: function (request, next) {
        switch (request.action) {
            ...
            case "tabs_remove_create_indexOf"://删除后创建再查找(返回网页内容)
                common_executeScript.tabs_remove_create_indexOf.a01(request.index, request.windowId, request.url, request.htmlArr, request.isHighlightTab, next);
                break;
...
//     common_executeScript.tabs_remove_create_getHeaders.a01(request.index, request.windowId, request.url, request.filterUrlArr, request.isHighlightTab, next);
//     break;

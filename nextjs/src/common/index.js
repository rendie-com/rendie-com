...
export const index = {
    ...
    for_list: async function (oo, list) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (list[i].list && list[i].length != 0) {
                //返回结果有数据，运行list内的参数。  
                for (let j = 0; j < newList[i].length; j++) {
                    newList[i][j].list = await this.for_list(newList[i][j], list[i].list)
                }
            }
            else if (list[i].elselist && list[i].length == 0) {
                //返回结果没数据，运行elselist内的参数。    
                for (let j = 0; j < newList[i].length; j++) {
                    newList[i][j].list = await this.for_list({}, list[i].elselist)
                }
            }
        }
        return newList;
    ...
    switch_action: async function (o1, oo) {
        switch (oo.action) {
            ...
            case "upload": oo = await this.upload(oo.path); break;

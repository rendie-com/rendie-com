...
var fun =
{
    ...
    d01: function () {
        let data = [{
            ...
            sql: "select @.fromid as fromid,@.proid as proid FROM @.table" + this.b01(this.obj.mode) + "  order by @.self_uptime desc limit 1",
        }]
        if (this.obj.A2 == 0) {
            data.push({
                action: "sqlite",
    ...
    d07: function (t, oo) {
        ...
        let unit = (sku.sellunit ? '按' + sku.sellunit + '起批1' + sku.sellunit + '=' + sku.scale : '')
        let html = '\
    ...
    e05: function (data, oo) {
        ...
        gg.postHtml(url, JSON.stringify(data), this.e06, this, o1)
    },
    e06: function (t, oo) {
        if (t.msg == "success") {
            ...
            let data = [{
                action: "sqlite",
                database: "shopee/商品/店铺商品/" + obj.params.site,
                sql: 'update @.table set @.self_uptime=@.uptime-1,@.scale=' + oo.scale + ',@.unitWeight=' + oo.unitWeight + ',@.MinimumOrder=' + oo.min_purchase_limit + ',@.discount=' + oo.discount + ' where @.fromid=' + oo.shopPro_fromid,
            }]

    ...
    common_price://价格
    {
        ...
        a02: function (oo) {
            ...
            oo.dom.append('\
                ...
                <tr><td class="right">1688单件购买数量：</td><td colspan="2">' + oo.sku.scale + '（一个属性的购买数量）</td></tr>\
            ...
            ');
            let min_purchase_limit = (oo.sku.scale > 1 ? Math.ceil(MaxCurrentPrices.beginAmount / oo.sku.scale) : MaxCurrentPrices.beginAmount)//最低购买数量
            if (oo._1688_freight > 0) {
                //有运费的，就改【起订量】和【原价】，把运费要回来。
                this.a03(min_purchase_limit,MaxCurrentPrices, oo)
            }
            else {
                //oo.sku.scale 是什么？答：采购时，需按这个倍数增加数量。
        ...
        a03: function (min_purchase_limit,MaxCurrentPrices, oo) {
            ...
            let promote_num = this.b04(MaxCurrentPrices.price * oo.sku.scale, min_purchase_limit, oo._1688_freight);//提升价格来要回运费，最多提升50%
            /////////////////////////////////////////////
            if (promote_num == 0) {
                let upPrice = MaxCurrentPrices.price * 0.5
                let maxLimit = this.b01((MaxCurrentPrices.price + upPrice) * oo.sku.scale, min_purchase_limit, oo._1688_freight)//提升价格要不到运费,那就提升【最低购买数量】加【提升50%价格】来要回来。
                if (maxLimit == 0) {
        ...
        a04: function (upPrice, min_purchase_limit, normal_price, oo) {
            ...
            let price01 = (normal_price + upPrice) * oo.sku.scale * oo.siteObj.exchangeRate
           ...
           let MinimumOrder = this.b02(min_purchase_limit, discountPrice, oo.siteObj.fullPrice, oo.siteObj.taxRate);//免运需要的【最低购买数量】
            ...
            let Weight = oo.ManualReview_1688_unitWeight * 1000 * MinimumOrder * oo.sku.scale//重量
            let freight = Tool.logistics.b03(Weight, oo.logistics, "Seller")//跨境物流成本[藏价]
            //免运价[不含平台费] = (全球商品价格 + 上调价格) * 汇率 * (件数 * 件倍数)
            let price02 = ((normal_price + upPrice) * oo.siteObj.exchangeRate) * MinimumOrder * oo.sku.scale
            //采购成本 = (采购价 * (件数 * 件倍数) + 运费 + 贴单费) * 汇率
            let price03 = (normal_price * (MinimumOrder * oo.sku.scale) + oo._1688_freight + 3) * oo.siteObj.exchangeRate
            /////////////////////////////////////////////////////
            ...
            oo.dom.append('\
                ...
                <tr>\
                    ...
                    <td colspan="2">\
                        ...
                        = ('+ normal_price + ' + ' + upPrice.toFixed(4) + ') * ' + oo.siteObj.exchangeRate + ' * ' + oo.sku.scale + '<br/>\
                ...
                <tr>\
                    ...
                    = ('+ normal_price + ' * ' + MinimumOrder + ' * ' + oo.sku.scale+' + ' + oo._1688_freight + ' + 3)  * ' + oo.siteObj.exchangeRate + '<br/>\
            ...
            for (let i = 1; i < len; i++) {
                if (oo.sku.currentPrices[i].beginAmount > min_purchase_limit && parseFloat(oo.sku.currentPrices[i].price)  < normal_price ) {
                    ...
                    if (max_count < 999999) {//shopee有这个限制
                        ...
                        //        "max_count": max_count,
                        //        "input_price": this.b03((parseFloat(oo.sku.currentPrices[i].price) + upPrice) * oo.sku.scale, oo.siteObj, oo.discount).toFixed(2)
                        //    }
                        //)
                    }
                }

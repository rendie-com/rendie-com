...
var fun =
{
    ...
    a01: function () {
        let html = Tool.header("Shopee &gt; 商品列表 &gt; 图片 &gt; 把【使用1688属性图】上传到Shopee平台") + '\
        <div class="p-2">\
    ...
    b03: function (attrPic_shopee, skuProps) {//图片是否都能找到
        let isBool = false;
        if (skuProps) {//值为【undefined】，可能是单价格。
            if (attrPic_shopee != 0) {
    ...
    /////////////////////////////////////////
    d01: function () {
        //@.ManualReview_1688=1        表示【使用1688属性图】
        // and @.fromid=652505949599
        let str = '\
        {\
            <r:product size=1 page=2 db="sqlite.1688" where=" where @.ManualReview_1688=1">\
                ...
                <r:prodes db="sqlite.1688_prodes/<:ManualReview_1688_fromid Fun=ProidNum(+$1,99)/>" size=1 where=" where @.fromid=\'<:ManualReview_1688_fromid/>\'">\
                    "sku":<:sku tag=0/>,\
                    "attrPic_shopee":<:attrPic_shopee tag=0/>,\
                    "attrPic":<:attrPic tag=0/>,\
                </r:prodes>\
            </r:product>\
    ...
    d03: function (oo) {
        ...
        $("#state").html("正在检查内容...");
        if (this.b03(oo.attrPic_shopee, oo.sku.skuProps)) {//图片是否都能找到
            $("#state").html("可以跳过")
            this.f02("ok");
        }
        else {
            if (oo.sku.skuProps) {
                this.d04(oo.sku.skuProps, oo.fromid)
            }
            else {
    ...
    d04: function (skuProps, fromid, attrPic_shopee) {
        $("#state").html("正在找图片...");
        let imgArr = []
        for (let i = 0; i < skuProps.length; i++) {
            let isImg = false;
    ...
    e06: function (t, oo) {
        let str = '\
        <if Fun(Db(sqlite.shopee_img,select count(1) from @.attrPic_1688 where @.src=\'' + oo.src + '\',count))==0>\
            <r: db="sqlite.shopee_img">insert into @.attrPic_1688(@.fromid,@.src,@.hash,@.addtime,@.width,@.height,@.size)values(' + this.obj.Barr.fromid + ',\'' + oo.src + '\',\'' + t + '\',' + Tool.gettime("") + ',' + oo.width + ',' + oo.height + ',' + oo.size + ')</r:>\
        </if>';
        Tool.ajax.a01('"ok"' + str, 1, this.e07, this);
    },
    e07: function (t) {
        if (t == "ok") {
            this.obj.B1++;
    ...
    f01: function () {
        $("#state").html("正在更新数据...");
        let str = '<r: db="sqlite.1688_prodes/' + Tool.remainder(this.obj.Barr.fromid, 99) + '">update @.prodes set @.attrPic_shopee=' + Tool.rpsql(JSON.stringify(this.obj.Barr.attrPic)) + ' where @.fromid=' + this.obj.Barr.fromid + '</r:>'
        Tool.ajax.a01('"ok"' + str, 1, this.f02, this)
    },
    f02: function (t) {
        if (t == "ok") {
            $("#state").html("下一步...");
